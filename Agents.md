...
3. **アラーム発動**: 設定時刻にアプリ内タイマーが自動検知 → 無音ループから有音アラームに切替、1分おきに鳴り続ける
...
3. **アラーム発動**: 設定時刻にアプリ内タイマーが自動検知 → アラーム音+画面点滅
...
- **自動アラーム発動**: アプリ内タイマー（`Timer`）が設定時刻到達を検知し、無音ループから有音アラーム（`alarm_sound.mp3`）に自動切替。ユーザー操作不要。
